...
router.post('/factura', async (req, res) => {
    try {
        ...
        const { cuit, puntoVenta, importeTotal, importeNeto, tipoFactura } = req.body;
        if (!cuit || !puntoVenta || !importeTotal || !importeNeto || !tipoFactura) {
            return res.status(400).json({
                success: false,
                message: 'Faltan datos requeridos (cuit, puntoVenta, importeTotal, importeNeto, tipoFactura)'
            });
        }
...
router.post('/factura/multiple', async (req, res) => {
    try {
        ...
        for (const factura of facturas) {
            const { cuit, puntoVenta, importeTotal, importeNeto, tipoFactura } = factura;
            if (!cuit || !puntoVenta || !importeTotal || !importeNeto || !tipoFactura) {
                respuestasAfip.push({ error: 'Faltan datos requeridos en una factura' });
                continue;
            }
