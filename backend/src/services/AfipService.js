...
class AfipService {
  ...
  async getExistingToken() {
    try {
      ...
      const tokenFiles = files
        .filter((file) => file.endsWith("loginTicketResponse.xml"))
        .sort()
        .reverse();
      ...
      if (tokenFiles.length > 0) {
        ...
        const content = await fs.readFile(
          path.join(this.basePath, lastFile),
          "utf8"
        );
        const result = await this.xmlParser.parseStringPromise(content);
        const loginCmsReturn =
          result["soapenv:Envelope"]["soapenv:Body"].loginCmsResponse
            .loginCmsReturn;
        ...
        const ticketResponse =
          await this.xmlParser.parseStringPromise(decodedXml);

        const tokenInfo = {
          generationTime:
            ticketResponse.loginTicketResponse.header.generationTime,
          expirationTime:
            ticketResponse.loginTicketResponse.header.expirationTime,
  ...
  async generateToken() {
    const seqNr = new Date()
      .toISOString()
      .replace(/[^0-9]/g, "")
      .slice(0, 14);
    const requestXml = path.join(
      this.basePath,
      `${seqNr}-LoginTicketRequest.xml`
    );
    const signedCms = path.join(
      this.basePath,
      `${seqNr}-LoginTicketRequest.xml.cms`
    );
    const base64Cms = path.join(
      this.basePath,
      `${seqNr}-LoginTicketRequest.xml.cms.base64`
    );
    const responseFile = path.join(
      this.basePath,
      `${seqNr}-loginTicketResponse.xml`
    );
    ...
    try {
      ...
      await execAsync(
        `openssl cms -sign -in "${requestXml}" -signer "${this.certificatePath}" -inkey "${this.privateKeyPath}" -nodetach -outform der -out "${signedCms}"`
      );
      ...
      while (attempt <= maxAttempts) {
        try {
          ...
          if (error.response && error.response.status === 500) {
            const fault = await this.xmlParser.parseStringPromise(
              error.response.data
            );
            const faultString =
              fault["soapenv:Envelope"]["soapenv:Body"]["soapenv:Fault"][
                "faultstring"
              ];
  ...
  async cleanOldTokenFiles() {
    try {
      const files = await fs.readdir(this.basePath);
      const tokenRelatedFiles = files.filter(
        (file) =>
 ...
 <soapenv:Body>
  ...
  async parsearUltimoComprobante(xmlResponse) {
    try {
      const result = await this.xmlParser.parseStringPromise(xmlResponse);
      const cbteNro =
        result["soap:Envelope"]["soap:Body"].FECompUltimoAutorizadoResponse
  ...
  generarXMLFactura(datos) {
    ...
    if (Math.abs(importeTotal - sumaComponentes) > 0.01) {
      console.warn(
        "Advertencia: La suma de componentes no coincide con ImpTotal por redondeo"
      );
    }
  ...
  async parsearRespuesta(xmlResponse) {
    try {
      const result = await this.xmlParser.parseStringPromise(xmlResponse);
      const response =
        result["soap:Envelope"]["soap:Body"].FECAESolicitarResponse
      ...
      return {
        ...
        observaciones: response.FeDetResp.FECAEDetResponse.Observaciones
          ? response.FeDetResp.FECAEDetResponse.Observaciones.Obs
          : null,
      };
    } catch (error) {
      console.error("Error parseando respuesta:", error);
