...
const PriceFactor = () => {
  ...
  const totalProductosVendidos = useSelector(
    (state: RootState) => state.data.totalProductosVendidos ?? 0
  );
  ...
  const calcularFactor = (ventas: number) => {
    ...
    if (!pricingStrategies[activeStrategy]) {
      console.error(
        `❌ Estrategia ${activeStrategy} no encontrada:`,
        pricingStrategies
      );
      return 1.0;
    }
    ...
    const factorIncremento =
      Math.pow(porcentajeAvance, currentStrategy.power) *
      currentStrategy.maxIncrease;
    ...
    <div className="bg-black flex flex-col justify-center items-center rounded-3xl pb-4 pt-4">
      ...
      <div className="flex flex-col text-center mt-6">
        ...
        <p className="text-xs text-gray-100 font-medium mb-1">
          {(isTestMode ? testHora : new Date().getHours()) >= 21 &&
          hasSetPrediction
            ? "Valor predicho"
            : "Valor exacto"}
        </p>
        ...
        <div className="px-4 w-full mt-4">
          <div className="w-full bg-gray-100 bg-opacity-50 rounded-full h-1 mt-4 mb-2 relative">
            ...
            >
              ...
              <div className="absolute right-0 -top-[5px] w-3.5 h-3.5 bg-gray-400  rounded-full"></div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="w-full px-4 mt-6">
        <button
          ...
          className="w-full py-2 bg-gray-800 rounded-lg text-gray-200  text-sm font-medium"
        >
          {showTestPanel
          ...
          <div className="mt-4 bg-gray-800 rounded-lg p-4">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <span className="text-gray-200 ">Modo test</span>
                <Toggle
                  isOn={isTestMode}
              ...
              <div className="pt-2 border-t border-gray-700">
                <h4 className="text-gray-200  text-sm mb-2">Casos de prueba</h4>
