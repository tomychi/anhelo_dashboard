  ...
  const handleMarkAsPaid = async (pedido: Order) => {
    ...
    try {
      const nuevoMetodoPago =
        pedido.metodoPago === "mercadoPago" ? "efectivo" : "mercadoPago";
      ...
      {showResults && searchResults.length > 0 && (
        <div className="absolute z-50 w-full mt-2 max-h-96 overflow-y-auto bg-white shadow-lg rounded-lg border border-gray-200">
          {searchResults.map((order) => (
            <div
              key={order.id}
              className="px-4 pb-4 pt-2 border-b hover:bg-gray-50"
            >
              ...
              {!order.elaborado && (
                <button
                  ...
                  className={`mt-4 bg-black w-full h-[64px] text-gray-100 rounded-lg flex justify-center font-bold items-center text-3xl font-coolvetica ${
                    order.cookNow
                      ? "bg-gray-200 text-red-main"
