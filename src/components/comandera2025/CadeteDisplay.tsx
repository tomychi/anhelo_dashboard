...
const CadeteDisplay: React.FC<CadeteDisplayProps> = ({
  ...
  const handleCancelarRecorrido = async (recorrido: RecorridoData) => {
    if (
      window.confirm("¿Estás seguro de que deseas cancelar este recorrido?")
    ) {
      try {
        const pedidosIds = recorrido.datosEstimados.pedidos.map((p) => p.id);

        await cancelCadetRecorrido(cadete.name, recorrido, pedidosIds);
      ...
      <div key={index} className="bg-gray-50 p-3 rounded-lg mb-2 text-sm">
        ...
        <div className="text-gray-600 mt-1">
          <div className="mb-1">
            ...
            {recorrido.datosEstimados.pedidos.map((pedido, idx) => {
              ...
              const horaLlegada = new Date(
                horaBase.getTime() +
                  (tiempoAcumulado + pedido.tiempoPercibido) * 60000
              );
              ...
              return (
                <div
                  key={idx}
                  className="ml-2 text-xs flex justify-between items-center"
                >
          ...
          <div className="flex justify-between text-xs text-gray-400  mt-1">
            <span>
              Distancia est.: {recorrido.datosEstimados.totalDistance}km
            </span>
          ...
          {isCompleted && recorrido.regreso && (
            <div className="text-xs text-gray-400  mt-1">
              Completado:{" "}
              {recorrido.regreso instanceof Date
                ? recorrido.regreso.toLocaleTimeString()
                : new Date(
                    recorrido.regreso.seconds * 1000
                  ).toLocaleTimeString()}
            </div>
          )}
        </div>
        ...
        <div className="mt-4 w-full">
          ...
          {completedRecorridos.length > 0 && (
            <>
              ...
              >
                <span>
                  Recorridos Anteriores ({completedRecorridos.length})
                </span>
                <svg
