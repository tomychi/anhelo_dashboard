	...
	return (
		<div>
			<div className="flex flex-row  justify-between">
				...
				<p className="text-xs border-2 p-1 border-white text-white w-full">{`${title}: ${tiempo} HS`}</p>
				...
				{!editandoTiempo && user.email !== "cadetes@anhelo.com" && (
					<button
						...
						className="text-xs border-2 text-white uppercase p-1"
					>
						Editar
					</button>
				)}
			...
			{editandoTiempo && (
				<form onSubmit={handleSubmit} className="w-full flex flex-row">
					<input
						...
						className="bg-white uppercase text-xs font-black w-full  text-green-500 p-1 border-2 border-white"
					/>
					<button
						type="submit"
						className="uppercase border-2 border-white text-xs text-white p-1"
					>
						Guardar
					</button>
				</form>
